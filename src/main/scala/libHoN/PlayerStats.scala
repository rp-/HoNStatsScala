...
class PlayerStats(playerData: scala.xml.Node) {
  ...
  def cacheEntry(conn: java.sql.Connection) = {
    if (!isCurrent(conn)) {
      val query = "INSERT INTO PLAYERSTATS ( aid, nickname, insertDate, xmlData) VALUES ( ?, ?, CURRENT_TIMESTAMP, ?);"
...
object PlayerStatsSql {
  def createTable(conn: java.sql.Connection) = {
    ...
    if (!rs.next()) {
      ...
      query.execute(
        """CREATE CACHED TABLE PLAYERSTATS (
  ...
  def getEntries(conn: java.sql.Connection, nicks: List[String]): List[PlayerStats] = {
    ...
    for (nick <- nicks) {
      val query = "SELECT xmlData FROM PlayerStats WHERE LCASE(nickname) = '" +
        nick.toLowerCase + "' AND id=(SELECT MAX(ID) FROM PlayerStats WHERE LCASE(nickname)='" + nick.toLowerCase + "')"
